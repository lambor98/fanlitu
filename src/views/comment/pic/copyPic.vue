<template>
    <div class="copyPic sonBox">
         <ul>
            <li>
                <p class="title">第一步：该返利商品为图文好评活动</p>
                <textarea v-model="txt" id="txt" placeholder="亲，请撰写返利商品的评价，评价内容要求至少15个字以上，商家会对评价进行评分，优质评价将提升您今后的中奖率"></textarea>
            ...
            <li>
                <p class="title">第二步：点击图片放大后保存图片，上传至淘宝/天猫评价中</p>
                <ul class="pic_list">
                    <li  v-for="(item,index) in picList" :key="index">
                        <img :src="picList[index]" alt="" @click="viewPic(index)">
                    </li>
                </ul>
            </li>
        </ul>
...
export default {
    ...
    methods:{
        ...
        submit(){  //提交评价
            if(this.imgName){
                axios.post(this.url+"/comment/submitCopypicEval",{order_id:this.info.order_id,content:this.txt,have_evaluation:this.imgName}).then(res=>{
                    if(res.data.status=="1000"){
                        Toast.success("提交成功")
    ...
    watch:{
        info(val){
            ...
            for(let i=0;i<val.comment_pic.length;i++){
                this.picList.push(this.imgHeader+val.comment_pic[i])
            }
        }
    },
    computed:{
